...
def main():
    ...
    args = kinova_utilities.parseConnectionArguments(parser)
    with kinova_utilities.DeviceConnection.createTcpConnection(args) as router:

        # 创建需要的服务
        base = BaseClient(router)
        base_cyclic = BaseCyclicClient(router)
        # Send_abosolute_cartesian_pose(base, base_cyclic,  [0.24, -0.06, 0.25,  175, 2, 91])
        # Move_to_zero_position(base)

        # Send_joint_speeds(base, [20, 0, 20, 0, 0, 0, 0])
        # time.sleep(5)
        # base.Stop()

        # Move_to_home_position(base)

        # Send_joint_angles(base)

        Send_relative_cartesian_pose(base, base_cyclic, [0.02, 0.03, 0.02, 0, 0, 0])

        # Send_twist_command(base, [0, 0, 0.05, 0, 0, 0])

        # Close_gripper(base)

        # Open_gripper(base)
        # Send_gripper_position(base, 0.5)

        print(*Get_cartesian_pose(base_cyclic))

        print(*Get_joint_positions(base, base_cyclic))

        # kTheta_x = 90.0
        # kTheta_y = 0.0
        # kTheta_z = 90.0
        # waypointsDefinition = ( (0.57,  -0.1,  0.42,  0.05, kTheta_x, kTheta_y, kTheta_z),
        #                         (0.57,   -0.2,  0.2, 0.05, kTheta_x, kTheta_y, kTheta_z),
        #                         (0.77,   -0.2,  0.1, 0.0, kTheta_x, kTheta_y, kTheta_z))
        # Send_Cartesian_waypoints(base, base_cyclic, waypointsDefinition)
